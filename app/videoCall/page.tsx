...
export default function VideoCallPage() {
  ...
  useEffect(() => {
    if (incomingCall && !isConnected && !isConnecting && !callId) {
      setIncomingCallData(incomingCall);
      setShowIncomingCall(true);
    }
  }, [incomingCall, isConnected, isConnecting, callId]);
  ...
  const handleRemoteStream = useCallback((stream: MediaStream) => {
    ...
    if (remoteVideoRef.current) {
      ...
      setConnectionStatus("Connected (with remote)");
    }
  }, []);
  ...
  const handleUserSelect = async (userEmail: string | undefined, userId: string) => {
    setShowUserDropdown(false);
    if (userEmail && userId) {
      await initiateCall(userId, userEmail);
    }
  };
  ...
  const endCall = async () => {
    if (callId) {
      try {
        await updateCall({ callId, payload: { status: "ended" } });
      } catch (err) {
